export const help = [
    {
        title: "Gettings started on Twelv12",
        content: "Learn how to create and collect artwork",
        image: "/images/help-pic-1.png",
        color: "#B9A9FB",
        url: "/help/category",
    },
    {
        title: "Create Collection & NFTs",
        content: "Learn how to create and collect artwork",
        image: "/images/help-pic-2.png",
        color: "#FFA6D6",
        url: "/help/category",
    },
    {
        title: "Buy & sell Artworks",
        content: "Learn how to create and collect artwork",
        image: "/images/help-pic-3.png",
        color: "#BCE6EC",
        url: "/help/category",
    },
    {
        title: "Gettings started on Twelv12",
        content: "Learn how to create and collect artwork",
        image: "/images/help-pic-4.png",
        color: "#1966F7",
        url: "/help/category",
    },
    {
        title: "Gettings started on Twelv12",
        content: "Learn how to create and collect artwork",
        image: "/images/help-pic-5.png",
        color: "#6DD3C2",
        url: "/help/category",
    },
];

export const categories = [
    {
        title: "Buying",
        links: [
            {
                title: "Connect your wallet to Twelv12",
                url: "/help/detail",
            },
            {
                title: "Guide to collection NFT artworks on Twelv12",
                url: "/help/detail",
            },
            {
                title: "Marketplace balance",
                url: "/help/detail",
            },
            {
                title: "Marketplace fees",
                url: "/help/detail",
            },
            {
                title: "What is offer price?",
                url: "/help/detail",
            },
        ],
    },
    {
        title: "Selling",
        links: [
            {
                title: "Connect your wallet to Twelv12",
                url: "/help/detail",
            },
            {
                title: "Guide to collection NFT artworks on Twelv12",
                url: "/help/detail",
            },
            {
                title: "Marketplace balance",
                url: "/help/detail",
            },
            {
                title: "Marketplace fees",
                url: "/help/detail",
            },
            {
                title: "What is offer price?",
                url: "/help/detail",
            },
        ],
    },
    {
        title: "Creating",
        links: [
            {
                title: "Connect your wallet to Twelv12",
                url: "/help/detail",
            },
            {
                title: "Guide to collection NFT artworks on Twelv12",
                url: "/help/detail",
            },
            {
                title: "Marketplace balance",
                url: "/help/detail",
            },
            {
                title: "Marketplace fees",
                url: "/help/detail",
            },
            {
                title: "What is offer price?",
                url: "/help/detail",
            },
        ],
    },
    {
        title: "Creating",
        links: [
            {
                title: "Connect your wallet to Twelv12",
                url: "/help/detail",
            },
            {
                title: "Guide to collection NFT artworks on Twelv12",
                url: "/help/detail",
            },
            {
                title: "Marketplace balance",
                url: "/help/detail",
            },
            {
                title: "Marketplace fees",
                url: "/help/detail",
            },
            {
                title: "What is offer price?",
                url: "/help/detail",
            },
        ],
    },
];

export const detail = [
    {
        image: "/images/help-photo-1.jpg",
        content: (
            <>
                <h3>Guide to collection NFT artworks on Twelv12</h3>
                <p>
                    We are laying the groundwork for web3 — the next generation
                    of the internet full of limitless possibilities. Join the
                    millions of creators, collectors, and curators who are on
                    this journey with you.
                </p>
                <p>
                    Ratione earum minus quia consectetur debitis magnam. Ex nemo
                    maiores reiciendis distinctio. Cupiditate nihil doloribus
                    repellat voluptatem.
                </p>
                <p>
                    Et id possimus harum ipsa eum odit labore. Eligendi repellat
                    impedit eaque corporis dolor ipsam vero. Vitae beatae
                    aperiam consequatur consequatur praesentium minus sequi
                    magni.
                </p>
                <p>
                    Et neque dolor velit aliquid earum reiciendis sequi harum.
                    Laudantium et doloremque deleniti quaerat. Dolorem molestias
                    doloribus. Non non laboriosam ut assumenda vel aut sequi.
                </p>
            </>
        ),
    },
    {
        image: "/images/help-photo-2.jpg",
        content: (
            <>
                <h3>Place a bid</h3>
                <p>
                    We are laying the groundwork for web3 — the next generation
                    of the internet full of limitless possibilities. Join the
                    millions of creators, collectors, and curators who are on
                    this journey with you.
                </p>
                <p>
                    Et neque dolor velit aliquid earum reiciendis sequi harum.
                    Laudantium et doloremque deleniti quaerat.
                </p>
                <ul>
                    <li>Connect your wallet to Twelv12</li>
                    <li>Guide to collection NFT artworks on Twelv12</li>
                    <li>Marketplace balance</li>
                    <li>Marketplace fees</li>
                    <li>What is offer price?</li>
                </ul>
            </>
        ),
    },
    {
        image: "/images/help-photo-3.jpg",
        content: (
            <>
                <h3>Buy now</h3>
                <p>
                    We are laying the groundwork for web3 — the next generation
                    of the internet full of limitless possibilities. Join the
                    millions of creators, collectors, and curators who are on
                    this journey with you.
                </p>
                <p>
                    Et neque dolor velit aliquid earum reiciendis sequi harum.
                    Laudantium et doloremque deleniti quaerat.
                </p>
                <ul>
                    <li>Connect your wallet to Twelv12</li>
                    <li>Guide to collection NFT artworks on Twelv12</li>
                    <li>Marketplace balance</li>
                    <li>Marketplace fees</li>
                    <li>What is offer price?</li>
                </ul>
            </>
        ),
    },
];
